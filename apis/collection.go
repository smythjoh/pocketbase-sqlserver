...
func collectionCreate(e *core.RequestEvent) error {
	...
	return e.App.OnCollectionCreateRequest().Trigger(event, func(e *core.CollectionRequestEvent) error {
		...
		// --- Dynamically build CREATE TABLE statement based on collection fields ---
		tableName := e.Collection.Name
		columns := []string{}

		for _, field := range e.Collection.Fields {
			var sqlType string
			switch field.Type() { // <-- call the function
			case "text":
			...
			columns = append(columns, "["+field.GetName()+"] "+sqlType)
		}

		createTableSQL := "CREATE TABLE [" + tableName + "] (" + strings.Join(columns, ", ") + ")"

		// connString := "sqlserver://sa:Basil99%2A@localhost:1433?database=pocketbase"
		connString := os.Getenv("SQLSERVER_CONN_STRING")
		sqlDB, err := sql.Open("sqlserver", connString)
		if err != nil {
			return e.BadRequestError("Collection created, but failed to connect to SQL Server: "+err.Error(), nil)
		}
		defer sqlDB.Close()

		if _, err := sqlDB.Exec(createTableSQL); err != nil {
			return e.BadRequestError("Collection created, but failed to create table in SQL Server: "+err.Error(), nil)
		}
		// --------------------------------------------------------------------------
